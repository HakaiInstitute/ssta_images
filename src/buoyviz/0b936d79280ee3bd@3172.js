  ...
  main.variable(observer("viewof limits")).define("viewof limits", ["brushFilterX","d3","dates","start","today"], function(brushFilterX,d3,dates,start,today)
{
  ...
  main.variable(observer()).define(["limits","datesToPlot"], function(limits,datesToPlot){return(
+limits[0] <= datesToPlot[263]
)});
  main.variable(observer()).define(["limits"], function(limits){return(
limits
)});
  main.variable(observer()).define(["datesToPlot"], function(datesToPlot){return(
datesToPlot[263]
)});
...
clickedSite
  ...
  main.variable(observer("lineChart")).define("lineChart", ["Plot","clickedSite","colors","time1","d3","width","dateForLabel","buoyClicked","siteClicked","colorView","md"], function(Plot,clickedSite,colors,time1,d3,width,dateForLabel,buoyClicked,siteClicked,colorView,md)
{
...
d3.max(clickedSite, (d) => d.sst) * 0.9
)});
...
buoys
  ...
  main.variable(observer("dateForLabel")).define("dateForLabel", ["clickedSite"], function(clickedSite)
{
  return clickedSite[Math.floor(clickedSite.length / 1.2)] === undefined
    ? 0
    : clickedSite[Math.floor(clickedSite.length / 1.2)].date;
}
);
