...
function Player(
  ...
  const btns = btnDesc.map(([x, kind]) => {
    ...
    return html`<button name=${x} class=button type=button >${icon}</button>`;
  });
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["md","f"], _26);
  ...
  main.variable(observer()).define(["md","g"], _28);
  main.variable(observer()).define(["md"], _29);
  main.variable(observer()).define(["html","letters","viewof f"], _30);
  main.variable(observer()).define(["html","viewof g"], _31);
