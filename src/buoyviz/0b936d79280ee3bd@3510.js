...
dates.indexOf(time1)
  ...
  main.variable(observer()).define(["width","htl"], function(width,htl){return(
htl.html`<style>
...
/*   input[type="range"][name="i"] {
  ...
  main.variable(observer("style")).define("style", ["html","width","ns"], function(html,width,ns){return(
html`
<style>
...
test === null
  ...
  main.variable(observer("test")).define("test", function(){return(
null
)});
...
Plot.legend({
  ...
  main.variable(observer("eventText")).define("eventText", ["d3","DOM","textures"], function(d3,DOM,textures)
{
  const width = 25,
    height = 25,
...
limits
  ...
  main.variable(observer("brushFilterX")).define("brushFilterX", ["d3","width","alldates","textures","hwEvent","Promises","Event"], function(d3,width,alldates,textures,hwEvent,Promises,Event){return(
function (
  extent,
  {
    defaultExtent,
  ...
  main.variable(observer("ns")).define("ns", ["Inputs"], function(Inputs){return(
Inputs.text().classList[0]
)});
  main.variable(observer("viewof colorView")).define("viewof colorView", ["Inputs","html"], function(Inputs,html){return(
...
colorView
  ...
  main.variable(observer("datesToPlot")).define("datesToPlot", ["datesCopy","limits"], function(datesCopy,limits){return(
datesCopy.filter((d) => d >= limits[0] && d <= limits[1])
)});
  main.variable(observer()).define(["d3","limits"], function(d3,limits){return(
...
[new Date("2013-01-01"), new Date()]
  ...
  main.define("initial firstLoad", function(){return(
0
)});
  ...
  main.variable(observer("viewof time1")).define("viewof time1", ["Scrubber","datesToPlot","width"], function(Scrubber,datesToPlot,width){return(
Scrubber(datesToPlot, {
  width: Math.min(555, width - 29),
  delay: 100,
  autoplay: false,
  loop: false,
  format: (d) => ""
...
limits
  ...
  main.variable(observer("viewof limits")).define("viewof limits", ["brushFilterX","d3","dates","start","today"], function(brushFilterX,d3,dates,start,today)
{
  ...
  main.variable(observer("curDate")).define("curDate", ["md","textcolor","timeFormat","time1"], function(md,textcolor,timeFormat,time1){return(
md`# ${textcolor(timeFormat(new Date(time1)), "black")}`
)});
  main.variable(observer("leg")).define("leg", ["colorView","Plot"], function(colorView,Plot)
{
...
8
  ...
  main.variable(observer("viewof lineChart")).define("viewof lineChart", ["Plot","clickedSite","colors","time1","d3","width","limits","yMaxDomainToUse","dateForLabel","yValueForLabel","buoyClicked","siteClicked","colorView","md"], function(Plot,clickedSite,colors,time1,d3,width,limits,yMaxDomainToUse,dateForLabel,yValueForLabel,buoyClicked,siteClicked,colorView,md)
{
  ...
  main.variable(observer()).define(["clickedSite"], function(clickedSite){return(
clickedSite.length
)});
  main.variable(observer()).define(["d3","clickedSite"], function(d3,clickedSite){return(
...
function Minimap(options) {
  ...
  main.variable(observer()).define(["buoys"], function(buoys){return(
buoys
)});
  main.variable(observer("yMaxDomainToUse")).define("yMaxDomainToUse", ["clickedSite","d3"], function(clickedSite,d3)
{
  return clickedSite[Math.floor(clickedSite.length / 1.2)] === undefined ?
    18 :  d3.max(clickedSite, (d) => d.sst) * 1.1
}
);
  main.variable(observer("yValueForLabel")).define("yValueForLabel", ["clickedSite","d3"], function(clickedSite,d3)
{
  return clickedSite[Math.floor(clickedSite.length / 1.2)] === undefined
    ? 18
    : d3.max(clickedSite, (d) => d.sst) * 1.1;
  ...
  main.variable(observer("dateForLabel")).define("dateForLabel", ["clickedSite","d3","limits"], function(clickedSite,d3,limits)
{
  return clickedSite[Math.floor(clickedSite.length / 1.2)] === undefined
    ? d3.timeDay.offset(limits[0], 30)
...
Math.floor(clickedSite.length * 0.1)
  ...
  main.variable(observer("buoyClicked")).define("buoyClicked", ["buoys","siteClicked"], function(buoys,siteClicked){return(
...
buoyDailyData.filter((d) => d.category === "Strong")
  ...
  main.variable(observer()).define(["dateExtent"], function(dateExtent){return(
