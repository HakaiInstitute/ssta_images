...
async function _limitsDelayed(Promises,limits)
{
...
function _currentPostion(brushFilterXDot,d3,dates,start,today){return(
brushFilterXDot(d3.extent(dates), {
...
function _26(width){return(
width
)}

function _27(hwEvent){return(
hwEvent
)}
...
function (
  ...
  svg
    ...
    // .data(fakeData)
    .data(hwEvent.filter((d) => new Date("201")))
    // .data(fakeData)
    .enter()
    .append("rect")
    .attr("x", (d) => x1(d.date))
...
function (
  ...
  function dragged(event, d) {
    // console.log("here", d3.pointer(event));
    ...
    if (isBelowMax && isAboveMin) {
      ...
      // console.log(event.x, x.invert(event.x), +x.invert(event.x));
      const ind = d3.bisect(datesToPlot, +x.invert(event.x));
      // console.log(ind, datesToPlot[ind]);
      // console.log(d3.bisect(datesToPlot, +x.invert(event.x)));
      // const ind = datesToPlot.indexOf(closest);
      // console.log(ind);
      $0.update(ind);
...
function _32(dates){return(
dates
)}

function _33(time1){return(
time1
)}

function _34(alldates){return(
alldates
)}
...
function _52(clickedSite){return(
...
function _table(Inputs,clickedSite){return(
...
function _coloursForSSTA(clickedSite,sstaColors){return(
...
function _67(buoyDailyData){return(
buoyDailyData
)}

function _71(time1){return(
...
function _73(buoyDailyData){return(
buoyDailyData.filter((d) => d.category === "Strong")
)}

function _74(dateExtent){return(
...
function _77(limits){return(
limits[0]
)}

function _dayBeforeStart(limits)
{
...
function _79(limits){return(
limits[0]
)}

function _81(limits){return(
limits
)}

function _82(dayBeforeStart){return(
dayBeforeStart
)}
...
function _99(opacityMHW){return(
opacityMHW("none")
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("limitsDelayed")).define("limitsDelayed", ["Promises","limits"], _limitsDelayed);
  ...
  main.variable(observer("viewof currentPostion")).define("viewof currentPostion", ["brushFilterXDot","d3","dates","start","today"], _currentPostion);
  main.variable(observer("currentPostion")).define("currentPostion", ["Generators", "viewof currentPostion"], (G, _) => G.input(_));
  ...
  main.variable(observer()).define(["width"], _26);
  main.variable(observer()).define(["hwEvent"], _27);
  ...
  main.variable(observer()).define(["dates"], _32);
  main.variable(observer()).define(["time1"], _33);
  main.variable(observer()).define(["alldates"], _34);
  ...
  main.variable(observer()).define(["colorView"], _37);
  ...
  main.variable(observer()).define(["limits"], _42);
  ...
  main.variable(observer()).define(["Plot"], _46);
  ...
  main.variable(observer()).define(["clickedSite"], _52);
  main.variable(observer("buoyClicked")).define("buoyClicked", ["buoys","siteClicked"], _buoyClicked);
  main.variable(observer()).define(["md"], _54);
  main.variable(observer("viewof table")).define("viewof table", ["Inputs","clickedSite"], _table);
  ...
  main.variable(observer("coloursForSSTA")).define("coloursForSSTA", ["clickedSite","sstaColors"], _coloursForSSTA);
  ...
  main.variable(observer()).define(["buoyDailyData"], _67);
  main.variable(observer()).define(["time1"], _71);
  main.variable(observer("HWsForDate")).define("HWsForDate", ["buoyDailyData","time1"], _HWsForDate);
  main.variable(observer()).define(["buoyDailyData"], _73);
  main.variable(observer()).define(["dateExtent"], _74);
  ...
  main.variable(observer()).define(["limits"], _77);
  main.variable(observer("dayBeforeStart")).define("dayBeforeStart", ["limits"], _dayBeforeStart);
  main.variable(observer()).define(["limits"], _79);
  main.variable(observer()).define(["limits"], _81);
  main.variable(observer()).define(["dayBeforeStart"], _82);
  ...
  main.variable(observer()).define(["opacityMHW"], _99);
