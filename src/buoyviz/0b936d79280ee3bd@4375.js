import define1 from "./d7a850070813db1b@990.js";
...
function _textures(require){return(
require("textures@1.2.0/dist/textures.js")
)}

function _8(Plot){return(
Plot.legend({
  marginLeft: "10",
  width: "240",
  color: {
    type: "categorical",
    domain: ["Notable heatwave events"],
    range: ["#cd3728"] // use the "accent" scheme
  },
  style: {
...
function _eventText(d3,DOM,textures)
{
  const width = 25,
    height = 25,
    margin = { top: 5, bottom: 5, left: 5, right: 5 };

  const sampleConfusionMatrix = { a: 25 };
...
function _11(limits){return(
limits
)}

function _12(datesToPlot){return(
datesToPlot
)}

function _14(time1){return(
new Date(time1)
)}

function _15(){return(
new Date(1617875774169)
)}

function _16(d3,datesToPlot){return(
d3.bisect(datesToPlot, 1578158090036)
)}

function _17(datesToPlot){return(
datesToPlot[0]
)}

function _18(d3,dates){return(
new Date(d3.extent(dates)[0])
)}

function _19(d3,dates){return(
d3.extent(dates)
)}

function _20(datesToPlot){return(
datesToPlot
)}

function _21(alldates){return(
alldates
)}

function _23(limits){return(
limits
)}
...
{ top: 4, right: 20, bottom: 30, left: 40 }
)}
...
function _32(brushedData){return(
brushedData
)}

function _33(d3,focus){return(
d3.timeMonth.floor(focus[1])
)}

function _34(focus){return(
focus
)}

function _35(d3,focus){return(
d3.timeDay.offset(focus[1], -1)
)}

function _36(brushedData){return(
brushedData
)}

function _37(brushedData){return(
brushedData[brushedData.length - 1]
)}

function _38(dateExtent){return(
dateExtent
)}

function _39(focus){return(
focus
)}

function _40(time1){return(
new Date(time1)
)}
...
function _chart(focusHeight,d3,width,margin,x,brusedAllDates,brushedData,sstaColors,time1,drag)
{
...
function _focus(d3,width,focusHeight,dateExtent,margin,xAxis,data,x1,y,sstaColors)
{
  const svg = d3
    .create("svg")
  ...
  function brushed({ selection }) {
    if (selection) {
      svg.property("value", selection.map(x.invert, x).map(d3.utcMonth.round));
      svg.dispatch("input");
    }
  }
...
function _49(d3,today){return(
d3.timeMonth.offset(today, -1)
)}

function _limits(brushFilterX,d3,dates,start,today)
{
...
function _brushFilterX(d3,width,alldates,textures,hwEvent,Event,brushHandle){return(
function (
  extent,
  {
    defaultExtent,
    margin = { top: 10, right: 20, bottom: 20, left: 95 },
    height = 50
  } = {}
) {
  const y = d3
  ...
  svg
    ...
    // .data(fakeData)
    .data(hwEvent.filter((d) => new Date("2010")))
    // .data(fakeData)
    .enter()
    .append("rect")
    .attr("x", (d) => x1(d.date))
    ...
    .attr("fill", (d) => textureOne.url());

  svg
    .selectAll("text")
    .data(["brush to", "filter dates"])
    .join("text")
    .text((d) => d)
    .style("font-size", 12)
    .style("fill", "#e0f5ee")
    ...
    .attr("dx", 10)
    .attr("dy", (d, i) => margin.top + 10 + i * 18);
  ...
  svg.append("g").call(xAxis).attr("class", "axisWhite");
  ...
  async function brushed(event) {
    ...
    // if (((s[1] - s[0]) / width) * 100 > 30) {
    //   b.call(brush.move, [previousS0, previousS1]);
    //   return;
    // }
    previousS0 = s[0];
    previousS1 = s[1];
    // await Promises.delay(1500, ""); // delay returning
    d3.select(this).call(brushHandle, event.selection, margin, height);
  }
...
function _brushFilterXDot(d3,width,time1,limits,datesToPlot,$0){return(
function (
  extent,
  {
    defaultExtent,
    margin = { top: 2, right: 20, bottom: 2, left: 95 },
    height = 25
  } = {}
) {
  const y = d3
  ...
  function draggedstarted(event, d) {

  }
  function dragged(event, d) {


    let isBelowMax = event.x >= x(+limits[0]);
    let isAboveMin = event.x <= x(+limits[1]);
...
FileAttachment("monthly@4.json").json()
)}

function _57(dates){return(
dates
)}

function _58(time1){return(
time1
)}

function _59(alldates){return(
alldates
)}
...
function _lineChart(Plot,clickedSite,colors,time1,d3,dateForLabel,yValueForLabel,buoyClicked,limitsDelayed,yMaxDomainToUse,siteClicked,colorView,md)
{
...
function _94(buoyDailyData){return(
buoyDailyData.filter((d) => d.category === "Strong")
)}

function _95(dateExtent){return(
...
function _100(limits){return(
limits[0]
)}

function _102(limits){return(
limits
)}

function _103(dayBeforeStart){return(
dayBeforeStart
)}
...
function _120(opacityMHW){return(
opacityMHW("none")
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["monthly@4.json",new URL("./files/12cc8b6bee4e42b74c5e9271d65d10b51ab00f9e2c50f0f2694462c552d0fd582daca4feb5255bbcee809431363dc2f954fad6b4d677c26b15a764ca04e750e6",import.meta.url)]]);
  ...
  main.variable(observer("textures")).define("textures", ["require"], _textures);
  main.variable(observer()).define(["Plot"], _8);
  main.variable(observer("eventText")).define("eventText", ["d3","DOM","textures"], _eventText);
  main.variable(observer("limitsDelayed")).define("limitsDelayed", ["Promises","focus"], _limitsDelayed);
  main.variable(observer()).define(["limits"], _11);
  main.variable(observer()).define(["datesToPlot"], _12);
  main.variable(observer()).define(["time1"], _14);
  main.variable(observer()).define(_15);
  main.variable(observer()).define(["d3","datesToPlot"], _16);
  main.variable(observer()).define(["datesToPlot"], _17);
  main.variable(observer()).define(["d3","dates"], _18);
  main.variable(observer()).define(["d3","dates"], _19);
  main.variable(observer()).define(["datesToPlot"], _20);
  main.variable(observer()).define(["alldates"], _21);
  const child1 = runtime.module(define1);
  main.import("Player", child1);
  main.variable(observer()).define(["limits"], _23);
  ...
  main.variable(observer()).define(["brushedData"], _32);
  main.variable(observer()).define(["d3","focus"], _33);
  main.variable(observer()).define(["focus"], _34);
  main.variable(observer()).define(["d3","focus"], _35);
  main.variable(observer()).define(["brushedData"], _36);
  main.variable(observer()).define(["brushedData"], _37);
  main.variable(observer()).define(["dateExtent"], _38);
  main.variable(observer()).define(["focus"], _39);
  main.variable(observer()).define(["time1"], _40);
  ...
  main.variable(observer("chart")).define("chart", ["focusHeight","d3","width","margin","x","brusedAllDates","brushedData","sstaColors","time1","drag"], _chart);
  main.variable(observer("viewof focus")).define("viewof focus", ["d3","width","focusHeight","dateExtent","margin","xAxis","data","x1","y","sstaColors"], _focus);
  ...
  main.variable(observer()).define(["d3","today"], _49);
  main.variable(observer("viewof limits")).define("viewof limits", ["brushFilterX","d3","dates","start","today"], _limits);
  main.variable(observer("limits")).define("limits", ["Generators", "viewof limits"], (G, _) => G.input(_));
  main.variable(observer("brushFilterX")).define("brushFilterX", ["d3","width","alldates","textures","hwEvent","Event","brushHandle"], _brushFilterX);
  main.variable(observer("brushHandle")).define("brushHandle", ["arc"], _brushHandle);
  main.variable(observer("arc")).define("arc", ["d3"], _arc);
  main.variable(observer("brushFilterXDot")).define("brushFilterXDot", ["d3","width","time1","limits","datesToPlot","viewof time1"], _brushFilterXDot);
  ...
  main.variable(observer()).define(["dates"], _57);
  main.variable(observer()).define(["time1"], _58);
  main.variable(observer()).define(["alldates"], _59);
  ...
  main.variable(observer()).define(["colorView"], _62);
  ...
  main.variable(observer()).define(["limits"], _67);
  ...
  main.variable(observer()).define(["Plot"], _70);
  ...
  main.variable(observer("viewof lineChart")).define("viewof lineChart", ["Plot","clickedSite","colors","time1","d3","dateForLabel","yValueForLabel","buoyClicked","limitsDelayed","yMaxDomainToUse","siteClicked","colorView","md"], _lineChart);
  ...
  main.variable(observer()).define(["time1"], _92);
  main.variable(observer("HWsForDate")).define("HWsForDate", ["buoyDailyData","time1"], _HWsForDate);
  main.variable(observer()).define(["buoyDailyData"], _94);
  main.variable(observer()).define(["dateExtent"], _95);
  ...
  main.variable(observer()).define(["limits"], _98);
  main.variable(observer("dayBeforeStart")).define("dayBeforeStart", ["focus"], _dayBeforeStart);
  main.variable(observer()).define(["limits"], _100);
  main.variable(observer()).define(["limits"], _102);
  main.variable(observer()).define(["dayBeforeStart"], _103);
  ...
  main.variable(observer()).define(["opacityMHW"], _120);
